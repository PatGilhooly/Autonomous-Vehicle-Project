    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          int cost[3] = {100, 80, 100}; // matrix to store costs 
          int k1 = 50; // results in collision
          int k2 = 100;
          int k3 = 1000;
          int k4 = 30;
          bool go_faster[3] = {1,1,1};
          bool slow_down[3] = {0,0,0};
          ...
          for (int i = 0; i < sensor_fusion.size(); i++) {
            ...
            // Figure out what lane each car is in
            int car_lane;
            int width = 4;
            if (0 <= d && d < width)
            {
            ...
            else if (width <= d && d < 2*width)
            {
              car_lane = 1; // Middle lane
            } 
            else if (2*width < d && d <= 3*width)
            {
            ...
            // safe distance from other cars is 30 meters
            int safe_dist = 30;

            // Detecting the state of the cars around the ego
            if ((check_car_s - car_s) < safe_dist && (check_car_s - car_s) > 0)
            {
              cost[car_lane] += k1;
              go_faster[car_lane] = 0;
              slow_down[car_lane] = 1;
              // std::cout<< "Car ahead!" <<std::endl;
            }


            if ((car_s - check_car_s) < safe_dist/2 && (car_s - check_car_s) > -10)
            {
              cost[car_lane] += k2;
            }
          ...
          int target = 0;
          int store_cost = 100000;
          // calcualte the current cost
          for (int i=0; i < 3; i++)
          {
              if (cost[i] < store_cost)
              {
                target = i;
                store_cost = cost[i];
              }
          ...
          for (int i = 1; i <= 50-previous_path_x.size(); i++) {
            ...
            // change speed to avoid collisions
            double acc = 0.224;
            double max_speed = 49.5; // speed below the threshold of 50

            // speed up or slow down
            if ((go_faster[target] == 1) && ref_vel < max_speed)
            {
              ref_vel += acc;
            }
            if ((slow_down[target] == 1) && ref_vel > 30)
            {
              ref_vel -= acc;
            }

            // Implement logic that tells car what to do
            if (target < lane)
            {
              lane--;
            }
